...
class Database(object):
    ...
    def modifyLogo(cls, info):
        sql = 'UPDATE table_account SET ' \
              'key_logo = \'' + info['imgid'] + '\' WHERE key_uuid = \'' + info['uuid'] + '\';'
    ...
    def addBookInfo(cls, info):
        ...
        #
        # print auths + '===' + tags
        # print auths.encode('UTF-8')

        # print info['auth']
        # print info['tags']
        # info['name'] = re.sub("'", "\'", info['name'])
        ...
        #       'VALUES (\'' + info['isbn'] + '\', \'' + info['lc'] + '\', \'' + info['name'] + '\', \'' + info['auth'] + '\', \'' + info['publisher'] \
        #       + '\', ' + str(info['edition']) + ', \'' + str(info['imgs']) + '\', \'' + info['auth'] + '\', \'' + info['abstract'] + '\');'
        ...
        # sql = re.sub('\'None\' | None', 'null', sql)
        try:
            cls.__cur.execute("INSERT INTO table_book_kind"
                  "(key_isbn, key_lc, key_name, key_auth, key_publisher, key_edition, key_imgs, key_tags, key_abstract)"
                  "VALUES(%s, %s, %s, %s, %s, %s, %s, %s, %s)"
                  ,(info['isbn'], info['lc'], info['name'], info['auth'], info['publisher'], info['edition'], info['imgs'], info['tags'], info['abstract']))
        except Exception, e:
            print e
            return {'status': 'failure', 'errorInfo': str(e)}
        finally:
    ...
    def searchISBN(cls, info):
        ...
        if info['type'] == 'name':
            # if info['value'].find("\\") :
                # print 'hahah-------------------------'
            sql = 'SELECT key_isbn FROM table_book_kind WHERE lower(key_name) like \'%' + info['value'].encode('UTF-8').lower() + '%\';'
            # else:
            #     print 'hahah----------'
            #     sql = 'SELECT key_isbn FROM table_book_kind WHERE lower(key_name) like \'%' + info['value'].lower() + '%\';'
            cls.__cur.execute(sql)
            rows = cls.__cur.fetchall()
            # ss = info['value'][0].encode('UTF-8')
            if (rows):
                isbn = []
                for row in rows:
                    isbn.append(row[0])
    ...
    def searchOperation(cls, uuid):
        ...
        # print rows,'in db'
        if (rows):
            return {'status': 'success', 'date': str(rows[0][0][0]), 'statuss': rows[0][1]}
        else:
            return {'status': 'failure', 'errorInfo': 'This uuid not exist!'}
    ...
    def addImage(cls, info):
        _uuid = str(uuid.uuid1())
        try:
            cls.__cur.execute('insert into table_image (key_uuid, key_mime, key_data) values (\'' + _uuid + '\', \'' + str(info['mime']) + '\', %s)',(psycopg2.Binary(info['data']),))
        except Exception, e:
            return {'status': 'failure', 'errorInfo': str(e)}
        finally:
            cls.__conn.commit()
    ...
    def createTable(cls):
        ...
        sql = 'INSERT INTO table_account (key_uuid, key_tel, key_user_name, key_password, key_register_date, key_right, key_stu_id) ' \
              'VALUES (\'' + str(uuid.uuid1()) + '\', \'13712347777\',\'rootroot\', \'0242c0436daa4c241ca8a793764b7dfb50c223121bb844cf49be670a3af4dd18\', \'' + str(datetime.datetime.now()) + '\', 2, 14130120111);'
        # print sql;
        try:
            cls.__cur.execute(sql)
        except Exception, e:
